...
class DecisionTree():
	...
	def learn(self, training_set,min_size,max_depth):
		...
		def get_child(ipdata):
			...
			for ix in decision_cols:
				ix_values = set(float(ix1[ix]) for  ix1 in ipdata)
		...
		def splitTree(main_tree,min_size,max_depth,curr_depth):
			...
			if len(left_child) <= min_size:
				...
				splitTree(main_tree['left_child'],min_size,max_depth,curr_depth+1)
			
			if len(right_child) <= min_size:
				main_tree['right_child'] = chk_last_node(right_child)
			else:
				main_tree['right_child'] = get_child(right_child)
				splitTree(main_tree['right_child'],min_size,max_depth,curr_depth+1)
		...
		splitTree(self.tree,min_size,max_depth,1)
...
def run_decision_tree():
	...
	for ix in list(range(10)):
		...
		min_size = 9
		max_depth = 30
		output = tree.learn(training_set,min_size,max_depth)
	...
	print("Mean Accuracy using 10 fold validation (with Pruning performed): " + str(accuracy))
